...
export class ChatGateway implements OnGatewayConnection, OnGatewayDisconnect {
  ...
  async handleConnection(socket: AuthenticatedSocket) {
    ...
    try {
      ...
      const ListOfAllUsersWithoutMe = ListOfAllUsers.filter(
        (u) => u.id !== user.id,
      );
      ...
      const DirectConversationObjectArray =
        await this.conversationService.getConversationByUserIdAndPrivacy(
          user.id,
          Privacy.DIRECT,
        );
  ...
  async reloadConversations(socket: AuthenticatedSocket, data: any) {
    ...
    try {
      ...
      const otherUsers = [];
      const conversation =
        await this.conversationService.getConversationWithParticipants(data.id);
      const ListOfAllUsers = await this.prisma.user.findMany();
      const ListOfAllUsersWithoutMe = ListOfAllUsers.filter(
        (u) => u.id !== user.id,
      );
      ...
      if (conversation.privacy === Privacy.DIRECT) {
        ...
        const ConversationObjectArray1 =
          await this.conversationService.getConversationByUserIdAndPrivacy(
            user.id,
            Privacy.PUBLIC,
          );
        const ConversationObjectArray2 =
          await this.conversationService.getConversationByUserIdAndPrivacy(
            user.id,
            Privacy.PRIVATE,
          );
        const ConversationObjectArray3 =
          await this.conversationService.getConversationByUserIdAndPrivacy(
            user.id,
            Privacy.PROTECTED,
          );
        ConversationObjectArray = ConversationObjectArray1.concat(
          ConversationObjectArray2,
          ConversationObjectArray3,
        );
      }
      const participants = [];
  ...
  async getGroupConversations(socket: AuthenticatedSocket) {
    ...
    try {
      ...
      const GroupConversationObjectArray1 =
        await this.conversationService.getConversationByUserIdAndPrivacy(
          user.id,
          Privacy.PUBLIC,
        );
      const GroupConversationObjectArray2 =
        await this.conversationService.getConversationByUserIdAndPrivacy(
          user.id,
          Privacy.PRIVATE,
        );
      const GroupConversationObjectArray3 =
        await this.conversationService.getConversationByUserIdAndPrivacy(
          user.id,
          Privacy.PROTECTED,
        );
      const GroupConversationObjectArray = GroupConversationObjectArray1.concat(
        GroupConversationObjectArray2,
        GroupConversationObjectArray3,
      );
      const ConversationObjectArrayWithParticipantId = [];
      const participants = [];
  ...
  async createConversation(socket: AuthenticatedSocket, data: any) {
    ...
    try {
      ...
      const participant =
        await this.participantService.createParticipantWithRole({
  ...
  async addUserToGroup(socket: AuthenticatedSocket, data: any) {
    ...
    try {
      ...
      const conversation =
        await this.conversationService.getConversationWithParticipants(
          data.conversationId,
        );
  ...
  async ListConversations(socket: AuthenticatedSocket) {
    ...
    try {
      ...
      const ConversationListPublic =
        await this.conversationService.getConversationByPrivacy(Privacy.PUBLIC);
      const ConversationListProtected =
        await this.conversationService.getConversationByPrivacy(
          Privacy.PROTECTED,
        );
      const ConversationList = ConversationListPublic.concat(
        ConversationListProtected,
      );
      const ConversationUserIsParticipantListPublic =
        await this.conversationService.getConversationByUserIdAndPrivacy(
          user.id,
          Privacy.PUBLIC,
        );
      const ConversationUserIsParticipantListProtected =
        await this.conversationService.getConversationByUserIdAndPrivacy(
          user.id,
          Privacy.PROTECTED,
        );
      const ConversationUserIsParticipantList =
        ConversationUserIsParticipantListPublic.concat(
          ConversationUserIsParticipantListProtected,
        );
  ...
  async joinConversation(socket: AuthenticatedSocket, data: any) {
    ...
    try {
      ...
      const conversation =
        await this.conversationService.getConversationWithParticipants(data);
